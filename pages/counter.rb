...
OLD_SKIP_CATEGORIES = OLD_SKIP_CATEGORIES_LINES.split("\n").select { |item| !(item =~ /^\s*$/ || item =~ /^\s*#/) }
...
def walk_categories( data, level )
  data.each do |k,v|
    ...
    ## next if  OLD_SKIP_CATEGORIES.include?( k )
